"""
synth_validation_scaffold.py  â€”  PHASE A: Synthetic Instrument-Validation
========================================================================
Purpose (AGENTS_v8 Â§9, ENGINEERING_v8 Â§7, RESEARCH_v8 Â§XI):
  ...
  This proves the instrument. It does NOT prove the phenomenon â€” no brain is
  involved. at no point report a synthetic recovery as proof intent geometry exists.

What is LIFTED, not reinvented (from the validated public-data harness â€” Tier 2):
...
THE FIREWALL (non-negotiable â€” RESEARCH_v8 Â§XI):
...
# â”€â”€ Schema constants (ENGINEERING_v8 Â§0; values are illustrative, fixed at PILOT) â”€â”€
...
N_PERMS = 1000       # RESEARCH_v8 Â§V floor â€” at no point below 1000

# â”€â”€ Calibration pool (nuisance realism) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Three calibration states, in increasing trustworthiness for SIZING the real run:
#   (0) None        â†’ generic Gaussian nuisance. Validates instrument BEHAVIOUR only;
#                     recovery-curve NUMBERS are NOT trustworthy for collection sizing.
#   (1) public MEG  â†’ real SPD nuisance STRUCTURE from saved public covariances. Better
#                     realism, but WRONG modality/dimensionality (MEG, not EEG montage).
#   (2) EEG pilot   â†’ real EEG nuisance. The ONLY state whose recovery-curve numbers may
...
# violation effect stays generic â€” at no point calibrated to the paradigm.
...
def load_calibration_pool(batch_dir, max_covs=3000):
    ...
    print(f"[calibration] pooled {len(pool)} real covariances "
          f"({'STATE 1: MEG-structure realism' if pool else 'STATE 0: none â†’ generic Gaussian'})")
    return len(pool)
...
# LIFTED BACKBONE  (verbatim from the validated harness â€” do not rewrite)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def per_subject_euclidean_alignment(cov):
    """Within-subject whitening (ENGINEERING_v8 Â§1). cov: (N,d,d) SPD for ONE subject."""
    R = cov.mean(axis=0)
    P = invsqrtm(R)
    return P @ cov @ P
...
def subject_gap(covs, gc):
    ...
    and oddball scored against the full congruent mean. (ENGINEERING_v8 Â§4)
    ...
    # Gate 0 â€” split-half reference stability (RESEARCH_v8 Gate 0), runs first
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# THE GENERATOR  (generic, firewalled â€” must NOT mirror the paradigm)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def make_subject_raw(n_chan, T, n_per_cond, effect, response_confound, motor_arm, rng,
    ...
    GENERIC model only:
      ...
      - VIOLATING trials get a generic separable covariance perturbation of size
        `effect` along a random low-rank direction â€” NOT a paradigm-shaped pattern.
      - response label is assigned INDEPENDENTLY of goal label (LAW 1 â€” orthogonal).

    EEG REALISM (ENGINEERING_v8 Â§0a) â€” models POST-preprocessing RESIDUAL artifact,
    i.e. what ICA/ASR leaves behind, NOT pristine Gaussian:
      artifact:      strength of residual ocular (frontal, low-rank, high-variance) +
                     ...
                     INDEPENDENTLY of condition (nuisance â€” firewall-safe).
    ...
    N = len(conds)
    resp = rng.integers(0, 2, size=N)                       # orthogonal to goal â€” LAW 1
...
def covs_from_raw(X, gc, estimator='erp'):
    """
    ENGINEERING_v8 Â§2/Â§4: scale â†’ covariance â†’ condition_spd â†’ EA.
    estimator='erp'  : ERPCovariances(congruent prototype) â€” more sensitive to elicited_response
    ...
    estimator='plain': Covariances â€” unbiased raw gap, may be less sensitive to elicited_response.
...
def check_negatives(n_subj, n_chan, T, n_per_cond, estimator='erp'):
    ...
    print(f"\nâ”€â”€ NEGATIVE CONTROLS  [estimator={estimator}]  (judged on bias-free ORDINAL) â”€â”€")
    ...
    # EEG residual artifact, independent of condition â†’ must stay null (nuisance only)
    d = line("residual-artifact null", run_cohort(n_subj, n_chan, T, n_per_cond, 0.0, artifact=1.5, estimator=estimator))
    # STRESS: artifact CORRELATED with violation â†’ instrument SHOULD show a (false) gap here.
    # This is the danger zone Â§0a names; it must be killed by the Gate-3 ocular covariate on
    ...
    print(f"  {'condition-leak STRESS':<24}: ordinal d={leak['mean_ord']:+.3f}  "
          f"â† EXPECTED non-null; on real data this MUST die under the ocular covariate (Gate 3)")
    print("  (raw gap under ERP is expected to be inflated by prototype membership;")
    print("   the ordinal is the readout that must be null on true nuisance â€” see report.)")
    return a and b and c and d

def recovery_curve(n_chan=16, T=120, effects=(0.0, 0.25, 0.5, 1.0, 2.0),
                   trial_counts=(15, 30, 60), n_subj=40, estimator='erp'):
    """
    The deliverable (RESEARCH_v8 Â§XI): ORDINAL detection reliability over
    (effect size Ã— trials/condition). This SIZES the real EEG collection.
    Reports the bias-free ordinal (violation vs oddball), not the biased raw gap.
    """
    print("=" * 66)
    print(f"RECOVERY CURVE [estimator={estimator}] â€” ORDINAL detection vs (effect Ã— trials)")
    ...
    print("  in-direction is the floor the real EEG paradigm must clear â†’ sets")
...
def main():
    ...
    print("=" * 66)
    print("PHASE A â€” SYNTHETIC INSTRUMENT VALIDATION")
    ...
    print(f"\nâ”€â”€ POSITIVE (effect=1.0, estimator=erp) â”€â”€")
    ...
    print(f"  (raw gap={pos['mean_d']:+.3f} â€” inflated by ERP prototype bias; ordinal is the truth)")
    ...
    # 3) the deliverable â€” adjudicate estimator via detection on the ordinal
    print()
    ...
    print("  ERP   = more sensitive to elicited_response signal, raw gap biased â†’ ordinal-only.")
