"""
v2.2_sliding_window_sweep.py — NSVD Riemannian TRDA Sweep
============================================================
Hyperparameter sweep over window_size and window_step for the
12-ROI sliding window architecture. Runs on 15-subject smart proxy.

MLOPS.md §1.2 compliance:
  - n_jobs=4 (BANNED: n_jobs=-1)
  - OMP_NUM_THREADS=1 to proccurrence OpenBLAS thread thrashing
  - Sequential sweep configs, parallel windows within each fold

rng_state = 42
"""

import os
# MLOPS.md §1.2 MANDATE: set before importing numpy/pyriemann
...
def execute_loso_fold(test_sid, subjects_data, window_size, window_step):
    ...
    # MLOPS.md §1.2: BANNED n_jobs=-1 (causes RAM lockup on /dev/shm).
    # Cap at n_jobs=4: enough to overlap Riemannian ops without thrashing.
    # loky backend avoids PyTorch fork-safety issues.
...
print("=" * 60)
print("CELL 6 & 7 — Hyperparameter Sweep")
print("=" * 60)

WINDOW_SIZES = [12, 25, 37, 50]
WINDOW_STEPS = [3, 6]
...
for w_size in WINDOW_SIZES:
    for w_step in WINDOW_STEPS:
        print(f"\n>>> Starting Sweep: Window={w_size} ({w_size/250*1000:.0f}ms), Step={w_step} ({w_step/250*1000:.0f}ms)", flush=True)
        sys.stdout.flush()
        
        peak_accs = []
        for fold_idx, (sid, _, _) in enumerate(subjects_data, 1):
            test_sid, accuracies, best_coef, best_window = execute_loso_fold(
                sid, subjects_data, w_size, w_step
            )
        ...
        print(f"  Sweep Result: {overall_mean*100:.2f}% (Window={w_size}, Step={w_step})")
        
        # Log to file
        with open(OUTPUT_JSONL, "a") as f:
            f.write(json.dumps({"window_size": w_size, "window_step": w_step, "mean_peak_acc": overall_mean}) + "\n")
            f.flush()
            os.fsync(f.fileno())
            
        if overall_mean > best_overall_acc:
            best_overall_acc = overall_mean
            best_overall_params = (w_size, w_step)

print("\n" + "=" * 60)
print(f"BEST CONFIGURATION: Window={best_overall_params[0]}, Step={best_overall_params[1]}")
print(f"BEST ACCURACY:      {best_overall_acc*100:.2f}%")
delta_pp = (best_overall_acc - 0.613) * 100
...
print("=" * 60)
print("Sweep complete. Download ml_sweep_results.jsonl from Kaggle outputs.")
