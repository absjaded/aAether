...
survives all alternative explanation":
  ...
  Guard 5 — 1000-shuffle cohort permutation null (§7)
...
# ── Cohort permutation null (§7) ──────────────────────────────────────────────

def cohort_permutation(loaded_results, n_perms=N_PERMS):
    """
    1000-shuffle cohort-tier permutation.
    Permutes Target/Lure labels within each subject, recomputes gap,
    aggregates across subjects. Power lives at n=75, not per-subject.
    """
    ...
    for p in range(n_perms):
        ...
        if (p + 1) % 100 == 0:
            print(f'    {p+1}/{n_perms} done', end='\r')
    print()
