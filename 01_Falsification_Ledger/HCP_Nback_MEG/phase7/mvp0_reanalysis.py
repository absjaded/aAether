...
def main():
    ...
    print(f'  Max per-subject |Δgap|    : {np.max(scale_delta_gap):.4f}')
    print(f'  Mean per-subject |Δgap|   : {np.mean(scale_delta_gap):.4f}')
    ...
    print('\n' + '=' * 66)
    print('HONEST VERDICT (per RESEARCH.md gates)')
    print('=' * 66)
    print(f'  Scale bug              : {"settled (harmless)" if settled else "MATERIAL — recheck"}')
    print(f'  Ordinal (NT≈T, L>NT)   : {"PASS" if ordinal_ok else "FAIL"}')
    ...
    print('  shows NT≈L>T and correct-only does not rescue a clean L>NT≈T, the')
